...
def main():
    ...
    for i in range(1,12):
        print(f'../data/2024-10-10/images/{i}/full_body/trimmed_image.png')
        frame = cv2.imread(f'../data/2024-10-10/images/{i}/full_body/trimmed_image.png')
        full_length, x_tail, contour, full_length_frame = get_full_length(frame.copy(),'./result',1,1,i)
        get_thin_point(frame.copy(),contour,x_tail,'./result',i)
        
def get_full_length(frame,folder_path, x_ratio, y_ratio,index):
    hsv_img = cv2.cvtColor(frame,cv2.COLOR_BGR2HSV)

    mask = cv2.inRange(hsv_img,np.array([0,0,0]),np.array([180,255,120])) #180, 255, 120
    ...
    if contours:
        ...
        cv2.imwrite(f'{folder_path}/full_length_{index}.png',frame)

        return dist, x_max[0], max_contour, frame
    else:
        print("No contours found")
        return None, None, None, None

def get_thin_point(frame,contour,x_tail,folder_path,index):
    ...
    try:
        ...
        if x_peak is not None and abs(thin_x - x_peak) > 50 and abs(thin_x - x_end) < 20:
            print(f'thin_x - x_peak, {thin_x}, {x_end}')
            thin_x = min_diff_x.get(frame,contour,x_start,x_peak+2,folder_path)
        
        cv2.putText(frame,f'thin point: {thin_x}', (1000,100), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,0,255),5)
        cv2.line(frame,(thin_x,0),(thin_x,frame.shape[0]),(0,0,255),2)
        cv2.imwrite(f'{folder_path}/thin_point_{index}.png',frame)
